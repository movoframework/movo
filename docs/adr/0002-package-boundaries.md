# ADR-0002 — Package boundaries, and why no `@movo/x402` package exists
...
The obvious first instinct — a `@movo/x402` package that owns "the protocol abstraction" for
...
**Eight core-track packages, three gated SCF-track packages, and no `@movo/x402`.**

### Core track — ships v0.1.0 regardless

| Package | Responsibility | Notably *not* its job |
|---|---|---|
| `@movo/core` | Config, resource model, compilation, errors, redaction, and the protocol narrow waist | Depending on any other `@movo/*` package; knowing about Express or the Stellar SDK |
| `@movo/server` | Compile Movo resources into an x402 `RoutesConfig` and mount it | Writing header encoding, 402 construction, or a lifecycle state machine |
| `@movo/stellar` | Preflight diagnostics and remediation hints | Defining Stellar constants, addresses, decimals or validators |
| `@movo/bazaar` | Derive discovery declarations; escalate upstream's soft-drop findings to build-time errors | Implementing its own validators |
| `@movo/client` | Stateful budget accounting, typed clients, outcome decoding | Generating or storing a key of any kind |
| `@movo/testing` | Facilitator fixtures, the failure matrix, harness, matchers | Being a runtime dependency of anything |
| `@movo/cli` | Commands; composition only | Containing check logic not exported by a library package |
| `create-movo-app` | Scaffolding | Runtime dependencies |

### SCF track — gated after M5, ships v0.2.0 or not at all

`@movo/facilitator`, `@movo/catalog`, `@movo/mcp`, and the deployable `apps/facilitator`.
...
A published `@movo/x402` façade would:
...
- `@movo/core` depends on no other `@movo/*` package. This keeps the dependency graph acyclic
...
Spec §3.1's "Depends on" column assigns `@x402/stellar` to `@movo/stellar`,
`@x402/extensions` to `@movo/bazaar`, and so on. At M0 those packages contain no code, and
under ADR-0004 they may not import `@x402/*` directly in any case — §3.1 itself notes for
`@movo/server` that its access is "via core waist re-export".
...
| `@x402/express` | 2.21.0 (exact) | Apache-2.0 | The mount point for `@movo/server` |
...
**A published `@movo/x402` façade package.** Rejected as pure cost — see above.
...
**Merging `@movo/stellar` into `@movo/core`, since it is only diagnostics.** Rejected for now:
`@movo/core` must not depend on `@stellar/stellar-sdk`, and preflight does. A rename to
`@movo/preflight` remains open (spec OQ-3) and must be decided before v0.1.0, since renaming
