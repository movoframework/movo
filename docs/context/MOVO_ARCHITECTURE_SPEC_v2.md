...
**Status:** Consolidated Revision — source of truth for implementation. Incorporates Spec Amendments 001–008 (see §A). Supersedes the 2026-08-09 original for all implementation purposes; the amendment files remain in `docs/context/` as the provenance trail.
**Date:** 2026-08-09 (original) · consolidated 2026-08-15
**Scope:** packages publish under `@movoframework/*` (Amendment 002)
...
## A. Consolidated Amendment Register (001–008) — READ FIRST

> **This revision folds Spec Amendments 001–008 into the spec.** The eight amendment files
> remain in `docs/context/` as the immutable provenance trail — they carry the incident
> narratives, the rationale, and the "what happened" record that produced each ruling. This
> register carries the *binding rulings themselves*, in spec-section order, and is normative.
> Where this register and the original body of a section disagree, **this register wins** and
> the relevant section has been edited to match (or flagged inline with `> AMENDED`).
>
> A fresh implementation session needs only this consolidated spec. The amendment files are for
> anyone asking *why* a decision was made, not *what* it is.

**Scope note (Amendment 002).** Packages publish under the **`@movoframework/*`** npm scope,
applied throughout this document. Everything else keeps the Movo name: the CLI binary is `movo`,
the config file `movo.config.ts`, environment variables `MOVO_*`, error codes `MOVO_E_*` /
`MOVO_W_*`, type names `MovoError` / `MovoResource` / `MovoConfig` and so on, and the scaffolder
`create-movo-app` (unscoped). The scope is a registry namespace, not the product name.

### A.1 Rulings by spec section

| Section | Binding ruling (current truth) | Source |
|---|---|---|
| §3.1 `core` deps | `@x402/stellar` is an **allowed runtime dependency** of `@movoframework/core`; `@stellar/stellar-sdk` may be present **transitively via `@x402/stellar`** but `core` must never import it **directly**. A CI check asserts no direct specifier, with a proof-of-failure fixture. | 003 §1 |
| §3.1 `server` deps | `@movoframework/server` additionally depends on `@movoframework/bazaar` (it registers `bazaarResourceServerExtension` behind `checkIfBazaarNeeded`). It still must **not** depend on `@movoframework/testing`. | 007 §6, 008; M5 |
| §3.1 `server` re-export | `@x402/express`-derived exports reach `server` at the **`@movoframework/core/server` subpath**, never on `core`'s main entry (which stays HTTP-framework-free) and never via a direct `@x402/express` import. | 004 §3 |
| §3.1 `core/protocol` | The narrow waist is organised as **two entry modules** (root + `server` subpath), both under `packages/core/src/protocol/`. It also re-exports the **`@x402/extensions/bazaar` surface** so `@movoframework/bazaar` reaches upstream validators lawfully. | 004 §3, 007 §4 |
| §5.1 `resolveConfig` | Signature is `resolveConfig(input?: ConfigLayers)` (five named precedence layers), **not** `Partial<MovoConfig>`. `MovoConfigInput` fields are typed `?: T \| undefined`. See ADR-0006. | 003 §2 |
| §5.1 config schema | `stellar.testnetFeeWorkaround` is **deleted** — the M0 spike proved the `fee:"1"` clone unnecessary against `@x402/*` 2.21.0 + the public testnet facilitator. If settlement fails in a fee-suggesting way, STOP and report a regression; do not reintroduce it. | 001 §1 (OQ-2) |
| §5.1 discovery validation | Movo does **not** author its own `serviceName`/`tags`/`iconUrl` validators and does **not** pre-validate `serviceName` (upstream's printable set rejects hyphen/underscore; a Movo pre-check would duplicate or disagree with upstream). Discovery problems surface as **build-time Findings** via `validateDiscoveryStrict`, escalating upstream soft-drops — not as bespoke config-time throws. | 003 §4, 007 §1, 008 §5 |
| §5.2 `MovoResource` | `settlementPolicy` is **deleted**. Upstream settles **after** the handler and cancels on throw or status ≥ 400; this is correct, not configurable through Movo's surface, and Movo **asserts** it rather than reimplementing it. `Price` is narrowed to `` `$${string}` `` \| `AssetAmount` (upstream `Money` also accepts `number`; the escape hatch is writing `accepts: PaymentOption[]` directly). `RouteConfig.resource` is omitted — upstream fills `url` from the request. | 001 §1, 003 §2 |
| §5.4 `MountOptions.facilitator` | Type is **`"config" \| FacilitatorClient`** only. The `"in-process"` / `"mock"` string shorthands are **removed** — `@movoframework/server` must not depend on `@movoframework/testing`, so the caller (the CLI, a test, an example) constructs a `MockFacilitator`/`InProcessFacilitator` and passes the **instance** in. | 005 §1 |
| §5.4 mount point | `mountExpress` uses **`paymentMiddlewareFromHTTPServer`**, not `paymentMiddlewareFromConfig` (the latter hides the `x402ResourceServer`, making the lifecycle hooks — where diagnostics live — unreachable). `server` constructs the `x402ResourceServer` itself and exposes it on `MountResult.server`. | 001 §2 |
| §5.7 `deriveDiscovery` | There is **one** upstream `declareDiscoveryExtension` that dispatches on whether `toolName` is present — **`declareMcpDiscoveryExtension` does not exist**. `deriveDiscovery(resource, resolvedConfig)` must do real Standard-Schema → JSON-Schema derivation, honour an explicit `inputSchema` override (with a test), and inherit project-level `serviceName`/`tags`/`iconUrl`. It checks `discovery.enabled` **structurally at compile time**; it must not call a request-time upstream helper (e.g. `checkIfBazaarNeeded`) during derivation, because `accepts` does not exist until request time. | 007 §3, 008 §1 |
| §5.7 `validateDiscoveryStrict` | Calls upstream validators and **consumes their `{valid, errors}` return** — never wraps them in a `try/catch` (upstream does not throw; a discarded return is a no-op posing as delegation). Escalates each soft-drop to an error-level `Finding` in the single `MOVO_E_*` registry. No `BAZAAR_E_*` namespace. | 007 §1–2, 008 §6 |
| §5.7 `readCatalogOutcome` | Returns the four-state union `success \| processing \| rejected \| unknown`. **Malformed base64 and absent header both resolve to `unknown` without throwing.** `unknown` is load-bearing; collapsing states into `undefined`/throw reintroduces a false failure signal. (This decoder fills a real gap: `@x402/core` has an internal `logExtensionResponsesHeader` but exports no public decoder — flagged as an upstream contribution.) | 007 §2, 008 §3 |
| §5.7 internal-host warning | `MOVO_W_DISCOVERY_INTERNAL_HOST` (a warning on internal/loopback hosts in `resource.url`) is **permitted and additive** — upstream inspects `iconUrl` but not the resource URL for internal hosts. Diagnostics upstream does not provide remain Movo's to add. | 008 §2 |
| §5.11 `createInProcessFacilitator` | Return type is **`FacilitatorClient`** only. **`asHandler()` is removed** — an HTTP `verify`/`settle`/`supported` surface is the entire facilitator wire contract and belongs to M6 behind the §26 gate, never to a testing convenience method. `withPaidServer` already takes a `FacilitatorClient` and is unchanged. `x402Facilitator.getSupported()` is **synchronous**; the factory wraps it to satisfy `FacilitatorClient`'s `Promise` return. | 006 §1, M3 |
| §5.11 scenario coverage | `InProcessFacilitator` is exercised by exactly the **five** signed-payload mutation scenarios (`wrongNetwork`, `wrongAsset`, `wrongAmount`, `expired`, `replayed`). The other four (`facilitator5xx`, `facilitatorTimeout`, `facilitatorMalformed`, `handlerFailureAfterVerify`) run against **`MockFacilitator` only** — a real facilitator cannot be told to fake being broken. AC3.2 is authoritative over any "all nine" prose. | 005 §2 |
| §5.9 hooks | Hook payloads carrying **live framework objects** (e.g. `onCompile` receiving the live `CompiledApp`) assert the non-leakage property **by test**, not by passing through `redact()` — redaction would render the handler map as `"[function]"` and make the hook's type a lie. | 003 §2 |
| §6.2 I2 | Verification failure → 402 whose **body is `{}`**; the rejection reason travels in the **`error` field of the re-issued `PAYMENT-REQUIRED` header**, not the response body. | 004 §1 |
| §6.2 I3/I4 | Confirmed at the **source level** (not merely observed): handler runs first with the response buffered; throw → `cancel("handler_threw")`, output discarded; settle failure → buffered output withheld. Movo asserts these; it does not build them. | 001 §1, 004 §2 |
| §6.2 I6 (new) | Handler returns 4xx → **not charged, no `PAYMENT-RESPONSE`** (upstream cancels settlement on status ≥ 400). A paid route that 404s costs the buyer nothing — a deliberate product property, documented in `docs/concepts/payment-lifecycle.md`. | 001 §1 |
| §6.2 `[VERIFY — OQ-1]` | **RESOLVED.** Settle-after-handler and I3/I4/I6 are confirmed; the `[VERIFY]` note is discharged. `SkipHandlerDirective` causes the handler to be **skipped**, never to run unverified, so `MovoPaymentContext.verified` stays the literal `true`. | 001 §1, 003 §3, 004 §2 |
| §5.6 preflight | `contract.Client` reads (SEP-41 decimals) are **diagnostics, not protocol** — no transaction, no signature, no fee — and are permitted in `@movoframework/stellar`. `TransactionBuilder` and `Keypair.fromSecret` remain violations. Preflight checks return Findings and never throw for a negative result; the **positive baseline** (a funded account with a valid trustline returns `ok`) is tested explicitly. | 004 §4, 003 §6 |
| §13 compatibility | New rows: streaming/SSE/chunked responses behind a paid route — **UNSUPPORTED** (upstream buffers the full response until settlement); pnpm 10.x **SUPPORTED**, 11.x **UNTESTED**. | 001 §1, 003 §5 |
| §12 / §5.10 docs URL | Error-docs URLs are built from a single exported `DOCS_BASE_URL` = `https://movoframework.github.io/movo`; never a repeated literal. M1 asserts every code resolves through it. | 002 §4 |
| §28 (M8) release | Release uses **npm OIDC trusted publishing** from GitHub Actions (`id-token: write`), **not** a long-lived automation token — npm is restricting 2FA-bypassing tokens (account changes Aug 2026, direct publishing Jan 2027). Configure trusted publishing per package before first publish; verify current requirements at `docs.npmjs.com`. | 002 §6 |

### A.2 Standing engineering rules earned across M0–M5

These live in `CONTRIBUTING.md`; summarised here because they govern how any section is implemented.

1. **No gate ships without a proof-of-failure test** (M0 `biome.json` truncation). *(001 §5)*
2. **Single-source every identifier a gate depends on** — derive gate patterns and fixtures from one constant (M0 scope-rename near-miss). *(002; M1)*
3. **Test the ordinary case, not only failures** — a negative-only suite passed while `defineApp` was broken for the common multi-resource case (`AnyMovoResource` variance). *(004 §6; M1)*
4. **A plausible fake is worse than a missing implementation** — verify a check fails when the real state is genuinely absent, not only that it passes on a plausible value (fabricated `ctx.payment`; a stubbed decimals check; an unconditional empty catalog). *(004 §6, 007 §1)*
5. **Delegation that discards its delegate's result is not delegation** — a `try/catch` around an upstream validator whose `{valid,errors}` return is thrown away calls upstream in source and does nothing in behaviour. *(008 §6a)*
6. **A correct answer to a question nobody asked is still scope creep** — deriving a route template from a concrete path, when `defineResource` paths are already templates, is dead code answering an invented question. *(008 §6b)*
7. **Nothing that parses or serialises the facilitator `verify`/`settle`/`supported` HTTP surface ships before M6**, regardless of which package it would live in or how small its surface. *(006 §3)*
8. **On conflict, STOP and report** — do not guess between two readings or silently redesign. This caught three real spec bugs before any code was written on top of them. *(005, 006)*

### A.3 Open items carried into M6+ (not yet closed)

- **Stock-client conformance suite (§1.16 layer 4).** Assigned to **M8**, where GATE 3's per-network-per-scheme matrix gives it coverage the e2e suite does not already provide. This is the explicit written resolution amendment 004 §8 / 008 §7 asked for — not another deferral. *(M5 report)*
- **`CODEOWNERS`** — create in M8. *(001 §4)*
- **`movo.dev` / `movoframework.dev` domain** — if obtained, change `DOCS_BASE_URL` (one line). Until then GitHub Pages under the org is the live URL. *(002 §4)*
- **Gated scaffold E2E** is intentionally out of the PR gate (it runs `npm install` against the network); template tests + no-install typecheck cover the compile path. *(M5 report)*
...
| G4 | Zero reimplementation of x402 or Stellar protocol primitives | Lint-enforced import boundary; no XDR or signature code in `@movoframework/*` |
...
Restated formally in §16. In summary Movo does not: reimplement x402; reimplement Stellar settlement; wrap `@x402/*` packages merely to rename their exports; vendor AGPL infrastructure; claim to be a Bazaar network by virtue of emitting metadata; create a `@movoframework/x402` package; take custody of funds; sign on behalf of buyers; or admit SCF-specific infrastructure into the core framework packages.
...
**D1 — Do not create `@movoframework/x402`.**
...
- **ALTERNATIVES CONSIDERED:** (i) A published `@movoframework/x402` façade — rejected: pure cost. (ii) Unrestricted direct imports everywhere — rejected: upstream drift would then produce compile errors in dozens of files across seven packages with no single place to absorb them.
...
**D2 — Delete `@movoframework/express`; do not wrap the middleware.**

- **DECISION:** Movo ships `@movoframework/server`, whose job is to *compile Movo resource modules into an `@x402/core` `RoutesConfig` and mount it via `@x402/express`'s existing `paymentMiddleware`*. Movo writes no HTTP header encoding, no 402 construction, and no lifecycle state machine.
...
- **CONSEQUENCES:** `@movoframework/server` is small — a compiler plus a mount function. Adapter breadth (Hono, Fastify, Next) becomes a question of which upstream adapter packages exist, not of Movo writing adapters.

---

**D3 — `@movoframework/bazaar` is a *derivation and enforcement* package, not a validation library.**
...
- **CONSEQUENCES:** `@movoframework/bazaar` is perhaps 300 lines. That is the correct size. The substantive Bazaar work is the catalog (M7), not the declaration.

---

**D4 — `@movoframework/stellar` is a diagnostics package, not a constants package.**

- **DECISION:** Movo does not define Stellar network identifiers, USDC addresses, decimals, passphrases, RPC URLs, address validators, or amount converters. All are imported from `@x402/stellar` `[FACT: all exported]`. `@movoframework/stellar` contains only preflight checks: account existence, trustline presence, asset resolution, facilitator/network agreement, ledger-based expiry headroom, and clock skew.
...
- **CONSEQUENCES:** `@movoframework/stellar` may be renamed `@movoframework/preflight` if the diagnostics framing proves clearer during M2. `[Open question OQ-3]`
...
| Rejected | Reason |
|---|---|
| `@movoframework/x402` package | `@x402/core` already owns it (D1) |
...
1. **Thin packages look unimpressive.** `@movoframework/bazaar` at ~300 lines and `@movoframework/stellar` as diagnostics-only will invite "is this a framework?" The honest answer is that the value is in the project layer and the diagnostics, and the docs must lead with that rather than with a package count.
...
| OQ-3 | Rename `@movoframework/stellar` → `@movoframework/preflight`? | M2 | Keep `@movoframework/stellar`; revisit before v0.1.0 since renaming after publish is a breaking change |
...
graph TB
    ...
    subgraph MOVOCORE["Movo core track"]
        COMPILE["@movoframework/core<br/>config resolution · resource compiler<br/>error registry · redaction<br/>protocol narrow waist"]
        SERVER["@movoframework/server<br/>mount compiled routes"]
        PRE["@movoframework/stellar<br/>preflight diagnostics"]
        BAZ["@movoframework/bazaar<br/>declaration derivation<br/>severity escalation"]
        TEST["@movoframework/testing<br/>mock + in-process facilitator<br/>failure matrix · matchers"]
        CLI["@movoframework/cli<br/>dev · doctor · test · bazaar"]
        CLIENT["@movoframework/client<br/>budget policy · typed clients"]
    end
    ...
    subgraph SCF["Movo SCF track (gated)"]
        FAC["@movoframework/facilitator"]
        CAT["@movoframework/catalog"]
        MCP["@movoframework/mcp"]
...
| **`@movoframework/core`** | Config resolution + provenance, resource → `RoutesConfig` compilation, error registry, redaction, the protocol narrow waist | Protocol types, HTTP, lifecycle, headers |
| **`@movoframework/server`** | Mounting compiled routes and handlers onto a Node HTTP framework | Middleware implementation, 402 construction, settlement ordering |
| **`@movoframework/stellar`** | Preflight diagnostics and their remediation hints | Network ids, asset addresses, decimals, signers, RPC clients |
| **`@movoframework/bazaar`** | Deriving declarations from resources; escalating soft-drop findings to build errors | Wire shapes, validators, sanitizers |
| **`@movoframework/client`** | Stateful spend accounting, typed resource clients, diagnostic decoding | Fetch wrapping, payment creation, signing |
| **`@movoframework/testing`** | Facilitator fixtures, failure matrix, matchers, harness | Verification, settlement |
| **`@movoframework/cli`** | Composition of the above into commands; scaffolding | Any business logic (all checks are library exports) |
...
|---|---|---|---|---|---|---|---|
| `@movoframework/core` | Config, resource model, compilation, errors, redaction, protocol waist | `defineConfig`, `defineResource`, `defineApp`, `compileApp`, `MovoError`, error registry, `redact` | `@x402/core` and `@x402/stellar` and `@x402/extensions` (only via `protocol/`), `zod` (matched major) | any other `@movoframework/*`; `express`; **direct** import of `@stellar/stellar-sdk` (transitive via `@x402/stellar` is allowed — 003 §1) | **Stable** | yes | core |
| `@movoframework/server` | Mount compiled routes onto a Node HTTP framework | `mountExpress`, `mountNodeHttp` | `@movoframework/core`, `@movoframework/core/server` subpath (`@x402/express` re-export — 004 §3), `@movoframework/bazaar` (registers `bazaarResourceServerExtension` — 007/M5), `express` (peer) | `@movoframework/stellar`; `@movoframework/testing` (005 §1); `@movoframework/cli`; any SCF package | **Stable** | yes | core |
| `@movoframework/stellar` | Preflight diagnostics + remediation hints | `preflight`, individual checks, `PreflightFinding` | `@movoframework/core`, `@x402/stellar`, `@stellar/stellar-sdk` | `express`; any SCF package | **Stable** | yes | core |
| `@movoframework/bazaar` | Derive declarations from resources; escalate soft-drop findings | `deriveDiscovery`, `validateDiscoveryStrict`, `queryCatalog` | `@movoframework/core`, `@x402/extensions` | own validator implementations; any SCF package | **Stable** | yes | core |
| `@movoframework/client` | Stateful budget accounting, typed resource clients, response decoding | `createBudget`, `createMovoClient`, `decodePaymentOutcome` | `@movoframework/core`, `@x402/fetch`, `@x402/stellar` (client subpath) | key generation or storage of any kind | **Experimental** in v0.1.0 | yes | core |
| `@movoframework/testing` | Facilitator fixtures, failure matrix, harness, matchers | `MockFacilitator`, `createInProcessFacilitator`, `withPaidServer`, `scenarios`, matchers | all core packages, `@x402/core`, `@x402/stellar` | being a dependency of any non-dev consumer | **Stable** | yes | core |
| `@movoframework/cli` | Commands; composition only | `movo` binary | all core packages | containing any check logic not exported by a library package | **Stable** | yes | core |
| `create-movo-app` | Scaffolding | `npm create movo-app` | none at runtime (templates reference published packages) | — | **Stable** | yes | core |
| `@movoframework/facilitator` | verify/settle/supported handlers, signer pool | `createFacilitator`, handlers | `@movoframework/core`, `@x402/core`, `@x402/stellar` | any AGPL package; XDR or signature code | **Experimental** | yes | **SCF** |
| `@movoframework/catalog` | Ingest, integrity, store, search | `createCatalog`, store port, search | `@movoframework/bazaar`, `@movoframework/facilitator` | any core package importing it | **Experimental** | yes | **SCF** |
| `@movoframework/mcp` | MCP discovery server | `createMcpDiscoveryServer` | `@movoframework/catalog`, `@movoframework/client`, `@modelcontextprotocol/sdk` | — | **Experimental** | yes | **SCF** |
| `apps/facilitator` | Deployable service | none (a service, not a library) | `@movoframework/{facilitator,catalog,mcp}`, `hono` | — | n/a | **no** (`private: true`) | **SCF** |
| `apps/docs` | Documentation site | none | — | — | n/a | **no** | core |
| `examples/*` | Reference projects | none | published Movo packages | — | n/a | **no** | core |

### 3.2 Strategies

**Publishing.** All `@movoframework/*` and `create-movo-app` publish to npm with provenance under a single Changesets-managed version line. `apps/*` and `examples/*` are `private: true`. SCF-track packages publish under the `next` dist-tag until the SCF gate passes.
...
**Test.** Vitest workspace. Four projects: `unit` (no network, `fetch` stubbed and asserted unused), `integration` (MockFacilitator), `e2e` (gated, real testnet), `conformance` (gated, third-party). Coverage floor 90% lines on `@movoframework/core`, 80% elsewhere; SCF packages 85% with the adversarial suite mandatory.
...
graph BT
    ...
    subgraph core["Movo core track — ships v0.1.0"]
        C["@movoframework/core"]
        S["@movoframework/server"]
        ST["@movoframework/stellar"]
        B["@movoframework/bazaar"]
        CL["@movoframework/client"]
        T["@movoframework/testing"]
        CLI["@movoframework/cli"]
        CMA["create-movo-app"]
    end

    subgraph scf["Movo SCF track — gated, ships v0.2.0 or never"]
        F["@movoframework/facilitator"]
        CAT["@movoframework/catalog"]
        M["@movoframework/mcp"]
...
|---|---|
| **Movo-owned code** | `@movoframework/{core,server,stellar,bazaar,client,testing,cli}`, `create-movo-app`, and on the gated track `@movoframework/{facilitator,catalog,mcp}` + `apps/facilitator` |
| **Official x402 code** | `@x402/core`, `@x402/stellar`, `@x402/express`, `@x402/extensions`, `@x402/fetch` — Apache-2.0, consumed, never forked |
| **Stellar SDK code** | `@stellar/stellar-sdk` — consumed by `@movoframework/stellar` for preflight and by `@x402/stellar` for settlement |
...
2. `@movoframework/core` depends on no other `@movoframework/*` package.
...
5. `@movoframework/testing` appears only in `devDependencies` of consumers.
...
export default defineConfig({
  ...
  stellar: {
    ...
    // AMENDED (001 §1 / OQ-2): testnetFeeWorkaround removed — the fee:"1" clone is
    // unnecessary against @x402/* 2.21.0 + the public testnet facilitator.
  },
});
```
...
// AMENDED (003 §2): input is ConfigLayers (five named precedence layers), not Partial<MovoConfig>;
// a single partial cannot express five layers, and two shapes sharing `env` would need runtime
// guessing in a money path. MovoConfigInput fields are typed `?: T | undefined`. See ADR-0006.
export declare function resolveConfig(input?: ConfigLayers): ResolvedConfig;
```
...
- `discovery` fields are **not** pre-validated by Movo-authored checks. AMENDED (007 §1, 008 §5): Movo passes `serviceName`/`tags`/`iconUrl` to upstream verbatim — upstream's printable set rejects hyphen/underscore, and a Movo-side pre-check would duplicate or, worse, disagree with upstream. Discovery problems surface as **build-time Findings** from `validateDiscoveryStrict` (§5.7), which escalates upstream soft-drops to error-level Findings in the `MOVO_E_*` registry — not as bespoke config-time throws.
...
**Backwards compatibility.** `CompiledApp.routes` is deliberately the raw upstream type, so a developer can bypass `@movoframework/server` entirely and pass it to `paymentMiddleware` themselves. This is the primary escape hatch and it is a stability promise.

### 5.4 Mounting — `@movoframework/server`
...
await mountExpress(server, app, {
  facilitator: "config",                // "config" | FacilitatorClient  (005 §1)
});
server.listen(4021);
```
...
interface MountOptions {
  // AMENDED (005 §1): "in-process" | "mock" shorthands removed. @movoframework/server must not
  // depend on @movoframework/testing, so the caller (CLI, test, example) constructs a
  // MockFacilitator / InProcessFacilitator and passes the instance in.
  facilitator?: "config" | FacilitatorClient;  // FacilitatorClient from @x402/core
...
**Semantics.** `mountExpress` calls `compileApp`, constructs an `x402ResourceServer` with the chosen `FacilitatorClient`, registers `ExactStellarScheme` from `@x402/stellar/exact/server` for the configured network, registers `bazaarResourceServerExtension` when any route declares discovery `[FACT: `checkIfBazaarNeeded` exists upstream for exactly this decision]`, applies **`paymentMiddlewareFromHTTPServer`** from `@x402/express` (AMENDED 001 §2 — passing the `x402ResourceServer` it constructed, because `paymentMiddlewareFromConfig` hides that server and makes the lifecycle hooks, where diagnostics live, unreachable), and then registers the plain route handlers. Movo writes no header code and no lifecycle code.
...
### 5.6 Preflight — `@movoframework/stellar`
...
### 5.7 Bazaar — `@movoframework/bazaar`

```ts
/** Derive an upstream discovery declaration from a compiled resource. AMENDED (007 §3):
 *  there is ONE upstream declareDiscoveryExtension that dispatches on whether toolName is
 *  present — declareMcpDiscoveryExtension does NOT exist. deriveDiscovery does real
 *  Standard-Schema -> JSON-Schema derivation, honours an explicit inputSchema override, and
 *  inherits project-level serviceName/tags/iconUrl. It checks discovery.enabled STRUCTURALLY
 *  at compile time; it must NOT call a request-time helper (checkIfBazaarNeeded) during
 *  derivation, because `accepts` does not exist until request time (008 §1). */
...
### 5.8 Client — `@movoframework/client`
 ...
 *  AMENDED (006 §1): returns FacilitatorClient ONLY — asHandler() is removed. An HTTP
 *  verify/settle/supported surface is the entire facilitator wire contract and belongs to M6
 *  behind the §26 gate, never to a testing convenience method. NB x402Facilitator.getSupported()
 *  is synchronous; the factory wraps it to satisfy FacilitatorClient's Promise return. */
...
| I2 | Verification failure → 402 with a non-null reason, handler not invoked. AMENDED (004 §1): the 402 **body is `{}`**; the reason is in the **`error` field of the re-issued `PAYMENT-REQUIRED` header**, not the body | M2 integration |
...
| I6 | Handler returns 4xx → **not charged, no `PAYMENT-RESPONSE`** (upstream cancels settlement on status ≥ 400). ADDED (001 §1): a paid route that 404s costs the buyer nothing — a deliberate product property | M2 integration |

`[RESOLVED — OQ-1]` (001 §1, 003 §3, 004 §2). Confirmed at the source level, not merely observed: the handler runs first with the response buffered; a throw triggers `cancel("handler_threw")` and discards the buffered output; status ≥ 400 triggers `cancel("handler_failed")`; settle failure withholds the buffered body. `SkipHandlerDirective` causes the handler to be **skipped**, never to run unverified, so `MovoPaymentContext.verified` stays the literal `true`. I3, I4 and I6 are upstream guarantees Movo **asserts** rather than implements. The original `[VERIFY]` obligation is discharged.
...
| Catalog ingest hook | `@movoframework/catalog`, mounted as a facilitator hook |
...
- `@movoframework/client` with a stateful budget accountant and typed `call()`
- `@movoframework/testing`: `MockFacilitator`, `InProcessFacilitator`, harness, nine-scenario matrix, matchers
...
12. **Documentation required.** `README`, `CONTRIBUTING` (narrow-waist rule, PR checklist), `CODE_OF_CONDUCT`, `SECURITY`, `SUPPORT` stub, `docs/COMPATIBILITY.md` (generated), ADRs 0001 (abstraction model), 0002 (package boundaries incl. why no `@movoframework/x402`), 0003 (facilitator composition), 0004 (narrow waist), and `docs/SPIKE_REPORT.md`.
...
2. **Objective.** Implement `@movoframework/core`: `defineConfig`/`resolveConfig` with provenance, `defineResource`/`defineApp`/`compileApp`, the error registry, redaction, and the protocol narrow waist — all pure, all testable without a network.
...
8. **Outputs.** `@movoframework/core` at ≥90% line coverage; `docs/reference/errors.md` generated; ADR-0005 (resource model), ADR-0006 (config precedence and provenance).
...
13. **Acceptance criteria.**
    ...
    - AC1.10 Line coverage for `@movoframework/core` ≥ 90%.
...
2. **Objective.** Ship `@movoframework/server` and `@movoframework/stellar`, mount a compiled app on Express and Node HTTP via upstream middleware, implement the six preflight checks, and prove a real settled testnet payment through the Movo stack.
...
13. **Acceptance criteria.**
    ...
    - AC2.7 `@movoframework/server` contains no header construction, no XDR, and no signature verification — asserted by a grep-based CI check for the terms `XDR`, `signAuthEntry`, `PAYMENT-REQUIRED` string literals outside tests.
...
2. **Objective.** Ship `@movoframework/testing`: `MockFacilitator`, `InProcessFacilitator` (real verification, real settlement, orchestrated locally), the harness, the nine-scenario failure matrix, and assertion matchers.
...
2. **Objective.** Ship `@movoframework/bazaar` (derive declarations from resources; escalate upstream soft-drops to build errors; query catalogs; interpret `EXTENSION-RESPONSES` honestly) and `@movoframework/client` (stateful budget accountant, typed `call()`).
...
1. **The API design in §5 rests on `[VERIFY]` items.** Settle ordering relative to the handler (OQ-1), the exact `ExactStellarScheme` constructor shapes, whether `paymentMiddleware` or `paymentMiddlewareFromConfig` is the right mount point, and whether the testnet fee workaround is still needed are all unknown from documentation alone. Designing `@movoframework/core`'s hooks, context types and error taxonomy against guesses would mean rewriting them.
...
| **What ships** | `@movoframework/*@0.1.0-alpha.x` and `create-movo-app@0.1.0-alpha.x` on the `alpha` tag. |
...
| **What ships** | `@movoframework/*@0.1.0` on `latest`; `create-movo-app@0.1.0`; the docs site; SCF-track packages on `next` until their own gate passes. |
...
| `@x402/express` | Exact pin, 2.21.0, peer `express ^4 \|\| ^5` `[FACT]` | **SUPPORTED** | Mount point for `@movoframework/server` |
...
| Edge runtime (Workers / Deno Deploy) | — | **UNSUPPORTED** | `@stellar/stellar-sdk` and `@movoframework/stellar` preflight assume Node APIs; not tested, not claimed |
| Browser | — | **UNSUPPORTED** for server packages; buyer client **EXPERIMENTAL** | `@movoframework/{core,server,stellar,testing,cli}` are Node-only by design. `@movoframework/client` has no inherent Node dependency but browser key handling is out of scope and untested. |
...
| `express` | HTTP framework for the example and `@movoframework/server` | *requires verification* (commonly MIT) | Peer / dev | **Yes**, pending verification | Peer dependency, consumer-supplied | Declared as a peer, not bundled |
...
J) ADRs in docs/adr/, each with Context / Decision / Consequences / Alternatives rejected:
   ...
   0002 package boundaries — why these packages, and explicitly why NO @movoframework/x402 package
...
- Do NOT create a @movoframework/x402 package.
...
OBJECTIVE
Implement @movoframework/core: the configuration system with provenance, the resource model, the
...
PREREQUISITE
M0 and M1 are complete. @movoframework/core can compile a resource declaration into an @x402/core
...
OBJECTIVE
Ship @movoframework/server (mounting) and @movoframework/stellar (preflight diagnostics), and prove a real,
...
A) packages/server
   ...
   @movoframework/server MUST NOT contain: header construction, 402 body construction, a lifecycle
...
E) tests
   ...
   middleware using @movoframework/core's MockFacilitator equivalent (a minimal FacilitatorClient
...
NON-GOALS FOR M2
   No Bazaar. No CLI. No @movoframework/client. No pubnet. No facilitator service. No catalog.
...
ACCEPTANCE CRITERIA — "M2 PASSED" MEANS ALL OF THESE
   ...
   AC2.7 a CI grep confirms @movoframework/server and @movoframework/stellar contain no XDR construction, no
...
OBJECTIVE
Ship @movoframework/testing so that the money path's failure modes are testable in CI without a funded
...
- @movoframework/testing may depend on any core-track package, but nothing may depend on it outside
  devDependencies.
...
A) packages/bazaar
   ...
   Then wire the extension into the 402 path: in @movoframework/server, register
...
A) packages/create-movo-app
   ...
   @movoframework/testing, .env.example, README.md with the exact next commands, and package.json with
...
B) packages/cli — four commands
   ...
   movo doctor [--json] [--check <id>...] [--fail-on warn|error]
     ...
     already be callable programmatically from @movoframework/stellar, @movoframework/core or @movoframework/bazaar. No
     check logic may live only in the CLI — if it does, it cannot be used in a downstream
     project's own CI.

   movo test [...vitest args] — a thin wrapper spawning Vitest with the @movoframework/testing setup
   ...
   movo bazaar validate | list | search — validate the project's discovery metadata; query a
     facilitator's catalog via @movoframework/bazaar's helpers.
...
**1. Objective.** An Apache-2.0, self-hostable Stellar x402 facilitator exposing `/verify`, `/settle` and `/supported`, composed from `x402Facilitator` + `ExactStellarScheme` (facilitator subpath), with a signer pool, channel accounts, metering, rate limiting and operational tooling — deployable on both `stellar:testnet` and `stellar:pubnet`, and packaged for **two deployment shapes**: a standalone `apps/facilitator` service, and **self-facilitation inside a resource server** (the upstream self-facilitation pattern), so an operator can run the facilitator in-process with their own paid API. Both shapes are committed deliverables (RFP §3.1). See RFP_COVERAGE_MAP.md §3.
...
**14. Upstream contribution requirements.** Any gap found in `@x402/stellar` during this work is reported upstream rather than shimmed permanently. The `upto` scheme is a **reasoned deferral to a committed phase 2, not an omission** — it requires authoring `scheme_upto_stellar.md` upstream through the x402 TSC plus (likely) a Soroban contract, a different risk class from the offchain facilitator; deferring it lets v0.1.0 ship the RFP's highest-value item (the Bazaar) sooner. The submission must state this reasoning explicitly (RFP_COVERAGE_MAP.md §2) and the scheme-registration extension point stays open. See §26 kill criteria.
...
- AC6.4 The x402 repository's own e2e suite passes against the service for both networks; results committed to `docs/CONFORMANCE.md`. **This is the RFP's literal acceptance mechanism (§3.6): reviewers point stock SDK code at the deliverable rather than read a conformance claim. Publish a settled transaction hash per network per scheme and assert a non-null reason on every rejection (AC6.5). Not internal hygiene — this is how the grant accepts the work.**
...
- AC6.11 **Smart-account support (RFP §3.1).** An unmodified stock client backed by a custom `__check_auth` contract account completes a payment on `stellar:testnet`, with an on-chain-confirmed hash — proving the facilitator accepts both classic keypairs and `__check_auth` accounts. Verify from the installed `@x402/stellar` facilitator declarations whether smart-account signers are accepted before assuming it; if upstream does not support them, STOP and report it as an upstream gap rather than reimplementing auth-entry validation.
- AC6.12 **Self-facilitation (RFP §3.1).** The facilitator runs in-process inside a resource server (not only as the standalone service) and a stock client completes a payment against that in-process deployment, demonstrated by test.

**16. Exit gate.** All twelve acceptance criteria (AC6.1–AC6.12), conformance evidence per network committed, runbook written and reviewed, Docker image builds and runs from the README alone.

**16a. Audit engagement (RFP §3.6) — start early, lands before the mainnet production tag.** The RFP requires a third-party security review via the SCF Audit Bank covering the settlement path, auth-entry validation, and the discovery trust boundary, before any `stellar:pubnet` production tag. This has external lead time. Open the engagement conversation during M6 rather than discovering it at release; the review itself gates the M8 production tag, not M6's exit. Recorded here so it is not found late.
...
**16. Exit gate.** All ten acceptance criteria; search-quality numbers and methodology published; a role-based developer guide (seller / buyer-and-agent / operator paths) with live testnet examples; two end-to-end example integrations; and the role-based guide **contributed upstream to Stellar Developer Docs** (RFP §5) — tracked as an external-contribution deliverable with its own done-state, since it depends on third-party review and merge, and therefore may land after the in-repo guide without blocking M7's exit.
...
> **AMENDED — read §A (Consolidated Amendment Register) first.** Rulings in §A are binding over anything below where they conflict. In particular: the `@movoframework/*` scope; the single `MOVO_E_*` registry; the narrow-waist and its `@x402/extensions/bazaar` re-export; no facilitator HTTP surface outside this track; and (M8) npm **OIDC trusted publishing**, not a long-lived token.
...
B) packages/mcp — an MCP discovery server exposing exactly three tools: bazaar.search,
   ...
   bazaar.paidCall uses @movoframework/client and MUST REQUIRE a budget policy — an agent-facing
...
| 3 | Run the **M1 prompt** (§19). Adjust §5's API design where the spike contradicted it. | Claude Code | `@movoframework/core` | AC1.1–1.10 |
| 4 | Run the **M2 prompt** (§20). | Claude Code | `@movoframework/server`, `@movoframework/stellar`, real settlement | **GATE 1** |
| 5 | **Human decision at GATE 1.** Confirm the on-chain hash. Reject any mocked evidence. | You | Proceed / stop | §12 |
| 6 | Run the **M3 prompt** (§21) and the **M4 prompt** (§22). These are parallelizable if two agents are available; otherwise M3 first, since M4's tests use M3's harness. | Claude Code | `@movoframework/testing`, `@movoframework/bazaar`, `@movoframework/client` | AC3.x, AC4.x |
...
- [ ] No `@movoframework/x402` package exists
...
- [ ] No XDR construction or signature verification anywhere in `@movoframework/*`, CI-grep enforced
...
- `@movoframework/x402`, or any package whose diff is mostly re-exports
