...
The Stellar Bazaar catalog: automatic cataloging at settle time, discovery listing and hybrid search.

The Stellar Bazaar catalog: automatic cataloging at settle time (no separate registration), discovery listing, and hybrid lexical + semantic search whose ranking quality is measured against a labelled set and gated in CI. SQLite and Postgres backends run the same suite.

## Install

```bash
pnpm add @movoframework/catalog
```
