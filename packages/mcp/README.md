...
An MCP discovery server over the Stellar Bazaar catalog: bazaar.search, bazaar.get and a budget-capped bazaar.paidCall.

An MCP discovery server over the Bazaar catalog: `bazaar.search`, `bazaar.get`, and `bazaar.paidCall`. `paidCall` requires a budget by construction — it takes the buyer's parts rather than a ready-made client, so there is no argument that yields a paid-call tool with no spend cap.

## Install

```bash
pnpm add @movoframework/mcp
```
